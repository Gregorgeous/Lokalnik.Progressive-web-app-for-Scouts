<template id="keysFinder">
  <div>
    <h3 class="text-xs-center title mt-2">
      Skontaktuj się z osobami które mają klucz do hufca. Obecnie są to:
    </h3>
    <div v-if="loadingState">
      <v-flex class="text-xs-center">
        ...
        <v-progress-circular class="amber--text" indeterminate size="100" v-else></v-progress-circular>
      </v-flex>
    </div>
    <div v-else>
      ...
      <currentKeyHolders v-else v-for="keyHolder in keyHolders" :key="keyHolder.name" :keyHolderName='keyHolder.name' :keyHolderSurname='keyHolder.surname'
        :keyHolderNote='keyHolder.keyHoldersNote'>
      </currentKeyHolders>
    </div>

    
    <v-bottom-nav absolute v-model="showBottomNav" class="">
    ...
    <v-dialog v-model="dialog" persistent max-width="500px">
      <v-card>
        ...
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap justify-center>
              <v-flex xs12>
                <v-text-field label='np. "mam klucze do 20 dnia 20.10"' v-model="newKeyHoldersNote"></v-text-field>
              </v-flex>
            </v-layout>
          </v-container>
        </v-card-text>
        <v-card-actions>
          ...
          <v-btn color="blue darken-1" flat @click='changeKeyHoldingState(true,newKeyHoldersNote)'>
            zapisz!
          </v-btn>
        </v-card-actions>
      </v-card>
