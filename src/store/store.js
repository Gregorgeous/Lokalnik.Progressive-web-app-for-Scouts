...
export const store = new Vuex.Store({
  ...
  mutations: {
      ...
      if (boolStatus.boolean ===false){
        ...
        if (state.user !== null && state.user.hasKeys !== undefined && state.user.keyHoldersNote !== undefined) {
          ...
          const myobject = {
            ...
            surname: state.user.surname, keyHoldersNote:state.user.keyHoldersNote };
    ...
    actions: {
      ...
      checkIfLoggedUser({commit,dispatch}, anotherAction){
        commit('changeLoadingState', true);
      ...
      changeKeyHoldingState({commit,state}, boolStatus){
      ...
      fetchCurrentEvents({commit}, index){
        ...
        .then((snap) => {
          ...
          commit('fetchEventsLocally', {myObject, index});
